...
class ImapSyncInteractor {
    ...
    private void updateHighestModSeqIfNecessary(Account account, String folderName, LocalFolder localFolder,
        ...
        updateHighestModSeq(account, folderName, localFolder, imapFolder.getHighestModSeq(), controller, syncHelper);
    }
    ...
    static void updateHighestModSeq(Account account, String folderName, LocalFolder providedLocalFolder,
            long highestModSeq, MessagingController controller, SyncHelper syncHelper) {
        LocalFolder localFolder = providedLocalFolder;
        try {
            if (localFolder == null) {
                localFolder = syncHelper.getOpenedLocalFolder(account, folderName);
            }
